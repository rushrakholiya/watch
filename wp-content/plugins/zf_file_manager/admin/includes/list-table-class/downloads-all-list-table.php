...
class downloads_All_List_Table extends WP_List_Table {
    ...
    function column_file_name($item) {
        $actions = array(
            'delete' => sprintf('<a href="?post_type=folder&page=%s&action=%s&download=%s">' . __('Delete', B5FILEMANAGER_PLUGIN_NAME) . '</a>', $_REQUEST['page'], 'delete', $item['ID'])
        );
    ...
    function no_items() {
        _e('No download logs found.', B5FILEMANAGER_PLUGIN_NAME);
    }
    ...
    function process_bulk_action() {
        ...
        if ('delete_all_logs' === $this->current_action()) {
            $del_all_download = $wpdb->query(
                'DELETE FROM ' . $wpdb->prefix . 'b5_file_manager_downloads;'
            );
    ...
    function prepare_items() {
        ...
        $data_results = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'b5_file_manager_downloads');
        $data = array();
        foreach ($data_results as $data_result) {
            $data[] = array('ID'=>$data_result->id, 'folder_name'=>$this->get_folder_name($data_result->folder_id), 'folder_id'=>$data_result->folder_id, 'file_name'=>$this->get_file_name($data_result->file_id), 'file_id'=>$data_result->file_id, 'visitor_ip' => $data_result->visitor_ip, 'date' => $data_result->date_time, 'visitor_name' => $data_result->visitor_name);
        }
