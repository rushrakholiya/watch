...
class downloads_Folder_List_Table extends WP_List_Table {

    var $folder_id;

    function no_items() {
        _e('No download logs found for this folder.', B5FILEMANAGER_PLUGIN_NAME);
    }
    ...
    function column_file_name($item) {
        $actions = array(
            'delete' => sprintf('<a href="?post_type=folder&page=%s&folder_id=%s&action=%s&download=%s">' . __('Delete', B5FILEMANAGER_PLUGIN_NAME) . '</a>', $_REQUEST['page'], $this->folder_id, 'delete', $item['ID'])
        );
    ...
    function process_bulk_action() {
        ...
        if ('delete_all_logs' === $this->current_action()) {
            $del_all_download = $wpdb->query(
                'DELETE FROM ' . $wpdb->prefix . 'b5_file_manager_downloads WHERE folder_id="'.$this->folder_id.'";'
            );
    ...
    function prepare_items() {
        ...
        $data_results = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'b5_file_manager_downloads WHERE folder_id="'.$this->folder_id.'"');
        $data = array();
        foreach ($data_results as $data_result) {
            $data[] = array('ID'=>$data_result->id, 'file_name'=>$this->get_file_name($data_result->file_id), 'file_id'=>$data_result->file_id, 'visitor_ip' => $data_result->visitor_ip, 'date' => $data_result->date_time, 'visitor_name' => $data_result->visitor_name);
        }
    ...
    function get_folder_name() {
        $folder_post = get_post($this->folder_id);
