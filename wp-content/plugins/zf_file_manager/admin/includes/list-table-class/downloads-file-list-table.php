...
class downloads_File_List_Table extends WP_List_Table {

    var $file_id;

    function no_items() {
        _e('No download logs found for this file.', B5FILEMANAGER_PLUGIN_NAME);
    }
    ...
    function get_columns() {
        $columns = array(
            ...
            'visitor_name'=>__('Username', B5FILEMANAGER_PLUGIN_NAME),
    ...
    function column_folder_name($item) {
        $actions = array(
            'delete' => sprintf('<a href="?post_type=folder&page=%s&file_id=%s&action=%s&download=%s">' . __('Delete', B5FILEMANAGER_PLUGIN_NAME) . '</a>', $_REQUEST['page'], $this->file_id, 'delete', $item['ID'])
        );

        return sprintf('<a href="?post_type=folder&page=%1$s&folder_id=%2$s">%3$s</a> %4$s',
            $_REQUEST['page'],
            $item['folder_id'],
            $item['folder_name'],
    ...
    function column_visitor_name($item) {
        return sprintf('<a href="?post_type=folder&page=%1$s&username=%2$s">%3$s</a>',
            $_REQUEST['page'],
            $item['visitor_name'],
            $item['visitor_name']
        );
    ...
    function process_bulk_action() {
        ...
        if ('delete_all_logs' === $this->current_action()) {
            $del_all_download = $wpdb->query(
                'DELETE FROM ' . $wpdb->prefix . 'b5_file_manager_downloads WHERE file_id="'.$this->file_id.'";'
            );
    ...
    function prepare_items() {
        ...
        $data_results = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'b5_file_manager_downloads WHERE file_id="'.$this->file_id.'"');
        $data = array();
        foreach ($data_results as $data_result) {
            $data[] = array('ID'=>$data_result->id, 'folder_name'=>$this->get_folder_name($data_result->folder_id), 'folder_id'=>$data_result->folder_id, 'visitor_ip' => $data_result->visitor_ip, 'date' => $data_result->date_time, 'visitor_name' => $data_result->visitor_name);
        }
