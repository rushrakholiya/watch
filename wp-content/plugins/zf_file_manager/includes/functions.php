<?php
    ...
    function b5_file_manager_redirect() {
        ...
        if (isset($_GET['action']) && $_GET['action'] == "zipcreate" && isset($_GET['b5-file']) && !empty($_GET['b5-file']) ) {

            //CREATE ZIP 
            //http://localhost/watch/test-file/?action=zipcreate&b5-file[941]=950&b5-file[857]=950           
            $zip = new ZipArchive();            
            $tmp_file = tempnam('.', '');
            ...
            $files = array();
            foreach($fileArray as $file_id=>$folder_id):
                $file_id = esc_attr($file_id);
                $folder_id = esc_attr($folder_id);
            ...
            endforeach;           
            $zip->close();            
            header('Content-disposition: attachment; filename="AllFile.zip"');
                    ...
                    <div class="%s">
                        ...
                        <div class="%s">
                            ...
                            <ul class="b5-infowrapper">
                                ...
                                '.$downloads_html.'
                                <li>
                                    <input type="checkbox" class="multiple-download" name="multipledownload[]" data-field_id="%s" data-folder_id="%s">

                                    <a class="%s%s%s%s" href="%s" data-field_id="%s" data-folder_id="%s" target="%s" title="%s">%s</a>
                                </li>
                            </ul>
                         </div>
                    </div>
