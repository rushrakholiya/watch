...
class WCFM_Marketplace {
    ...
    if( wcfm_is_vendor() ) {
    	...
    	// Booking Filter
			...
			add_filter( 'wcfm_valid_line_items', array( &$this, 'wcfmmp_valid_line_items' ), 10, 3 );
  ...
  function wcfmmp_after_line_total( $item, $order ) {
  	...
  	if( $item->get_product_id() ) {
		...
		?>
			<?php if ( $get_shipping = $WCFMmp->wcfmmp_vendor->is_vendor_get_shipping( $this->vendor_id ) ) { ?>
				<td class="line_cost no_ipad no_mob" width="1%">
					<div class="view"><?php 
					echo wc_price( $order_line_due[0]->total_shipping, array( 'currency' => $order_currency ) ); 
					if( $order_line_due[0]->is_refunded) {
						echo '<small class="refunded">-' . wc_price( $order_line_due[0]->total_shipping, array( 'currency' => $order->get_currency() ) ) . '</small>';
					}
					?></div>
				</td>
			<?php } ?>
			<?php if ( $WCFMmp->wcfmmp_vendor->is_vendor_get_tax( $this->vendor_id ) ) { ?>
				<td class="line_cost no_ipad no_mob">
					<div class="view"><?php 
					echo wc_price( $order_line_due[0]->tax, array( 'currency' => $order_currency ) ); 
					if( $order_line_due[0]->is_refunded) {
						echo '<small class="refunded">-' . wc_price( $order_line_due[0]->tax, array( 'currency' => $order->get_currency() ) ) . '</small>';
					}
					?></div>
				</td>
				<?php if( $get_shipping ) { ?>
					<td class="line_cost shipping_tax_cost no_ipad no_mob">
						<div class="view"><?php 
						echo wc_price( $order_line_due[0]->shipping_tax_amount, array( 'currency' => $order_currency ) ); 
						if( $order_line_due[0]->is_refunded) {
							echo '<small class="refunded">-' . wc_price( $order_line_due[0]->shipping_tax_amount, array( 'currency' => $order->get_currency() ) ) . '</small>';
						}
						?></div>
					</td>
				<?php } ?>
			<?php } ?>
			<?php if( apply_filters( 'wcfm_is_allow_total', true ) ) { ?>
			  <td class="line_cost total_cost no_ipad no_mob">
			    <?php 
			    echo wc_price( $line_total, array( 'currency' => $order_currency ) ); 
			    if( $order_line_due[0]->is_refunded ) { 
			    	echo '<small class="refunded">-' . wc_price( $line_total, array( 'currency' => $order->get_currency() ) ) . '</small>';
			    } elseif ( $refunded = $order->get_total_refunded_for_item( $item->get_id() ) ) {
  ...
  function wcfmmp_order_total_commission( $order_id ) {
    ...
    $total = $order_due[0]->line_total;
		...
		<?php if( !$admin_fee_mode ) { ?>
			<tr>
				<td class="label"><?php _e( 'Line Total', 'wc-frontend-manager' ); ?>:</td>
				<td>
